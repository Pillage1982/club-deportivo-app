...
CREATE TABLE eventos (
    id BIGINT AUTO_INCREMENT PRIMARY KEY,
    nombre VARCHAR(100) NOT NULL,
    tipo ENUM('entrenamiento','partido','reunion') NOT NULL,
...
CREATE TABLE asistencias (
    ...
    estado ENUM('presente','atrasado','ausente') NOT NULL,
...
INSERT INTO tipos_cuotas (nombre, monto_base) VALUES
('Mensualidad', 10000),
('Inscripción', 20000);
...
CREATE TABLE cuotas (
    ...
    origen ENUM('interno','externo') DEFAULT 'interno',
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP,

    FOREIGN KEY (persona_id) REFERENCES personas(id) ON DELETE CASCADE,
    FOREIGN KEY (tipo_cuota_id) REFERENCES tipos_cuotas(id)
);
...
CREATE TABLE pago_detalle (
    ...
    tipo ENUM('multa','cuota') NOT NULL,
...
CREATE INDEX idx_persona ON multas(persona_id);
CREATE INDEX idx_cuotas_persona ON cuotas(persona_id);
...
SELECT 
    ...
    COALESCE(m.total_multas, 0) AS deuda_multas,
    COALESCE(c.total_cuotas, 0) AS deuda_cuotas,
    COALESCE(m.total_multas, 0) + COALESCE(c.total_cuotas, 0) AS deuda_total
...
CREATE TABLE usuarios (
    ...
    rol ENUM('admin','tesorero','entrenador')
    DEFAULT 'admin'
);