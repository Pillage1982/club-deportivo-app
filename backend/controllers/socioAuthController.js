...
//
// A diferencia de calamena, aquí no hay emailService/nodemailer configurado (la junta
// todavía no tiene correo propio): el PIN se devuelve en la respuesta y el admin lo
// entrega a mano (Copiar / WhatsApp desde el frontend). Cuando exista un correo propio,
// se puede sumar el envío automático igual que en calamena sin tocar el resto del flujo.
  ...
  socioAuthModel.obtenerDatosPersonaBasico(personaId, (err, persona) => {
    ...
    bcrypt.hash(pin, BCRYPT_ROUNDS, (errHash, hash) => {
      ...
      socioAuthModel.asignarPin(persona.persona_id, hash, errAsignar => {
        ...
        // Sin emailService en esta rama: el PIN se entrega a mano desde el
        // frontend (Copiar / WhatsApp), no por correo.
        res.json({
...
// Uno por uno (no Promise.all): para una junta chica (decenas de personas) no
